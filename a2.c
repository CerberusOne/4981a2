...
int cleanup(void);
int open_semaphore_set(key_t skey, int numsems);
void wait(int);	//wait(sem)
void signal(int);	//signal(sem)
...
int main(int argc, char *argv[]){
	...
	key_t skey;
	int msg_id;
	int status;	//semaphore key, id and status structure

	extern cleanup();

	//remove semaphore if any interrupts occur
	for(int i = 0; i < 20; i++){
		//signal(i, cleanup);
	}

	//sid = initsem((key_t)semKey);
	open_semaphore_set(skey, 1);


	//Generate a IPC key value
	mkey = ftok (".", 'm');
...
void* checkQueue(void* msg_id){
	...
	while(1){
		...
		if(pthread_mutex_lock(&printfLock) == 0){
